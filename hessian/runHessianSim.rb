...
CREATE_HESSIAN_BS = File.expand_path("~/lab-tools/bs_inBV/latest/create_hessian_by_bootstrapping.rb")
...
#PHYLO_HESSIAN = File.expand_path("~/lab-tools/phyloHessian/phyloHessianWrapper.rb")
...
def run_phyloHessian(ori_fasta, ori_unrooted_tree, ph_sub_outdir, ref_tre, real_cpu, model, bd, bsn, clock, sim_ali_outdir, is_mwopt, cmd_out_fh, hessian_type)
  cmd_add = ''
  if ! is_mwopt
    cmd_add = '--no_mwopt'
  end
  ...
  cmd = "#{RUBY} #{PHYLO_HESSIAN} -s #{ori_fasta} -t #{ori_unrooted_tree} --outdir #{ph_sub_outdir} --force --reftree #{ref_tre} --cpu #{real_cpu} -m #{model} --bd #{bd} \
        --run_mcmctree --bsn #{bsn} --clock #{clock} --phylip #{sim_ali_outdir}/combined.phy --tree_indir #{sim_ali_outdir} #{cmd_add} --hessian_type #{hessian_type}"
  cmd_out_fh.puts cmd
  `#{cmd}`
end
...
def get_bs_models(models)
  ...
  bs_model_add_cmds = parse_bs_models( models.select{|i|i =~ /bs_inBV/i} )
  models.reject!{|i|i =~ /bs_inBV/i}
  return([models, bs_models, bs_model_add_cmds])
end
...
def parse_bs_models(models)
  models
    .select { |m| m =~ /bs_inBV/i }
    .map do |m|
      # Case 1: explicit form, e.g. bs_inBV.bf.npbs / bs_inBV.pbs
      if (md = m.match(/bs_inBV(?:\.(best_fit|bf))?\.(npbs|pbs)/i))
        best_fit = md[1] ? 'y' : 'n'
        bs = md[2].downcase
        "--bs #{bs} --best_fit #{best_fit}"

      # Case 2: only bs_inBV (e.g. LG+G.bs_inBV) -> default
      elsif m.match(/bs_inBV$/i)
        "--bs npbs --best_fit y"
...
def do_create_hessian(bs_models, bs_model_add_cmds, bootstrap, phylip, species_tree, mcmctree_ctl, ref_tre, cpu, outdir0, is_mwopt)
  # outdir0: dating/
  cmd_add = ''
  cmd_add = '--no_mwopt' if ! is_mwopt
  Parallel.map(0.upto(bs_models.size-1), in_threads:cpu) do |c|
    bs_model = bs_models[c]
    bs_model_add_cmd = bs_model_add_cmds[c]

    bs = bs_model_add_cmd[/--bs\s+(\S+)/, 1]          # e.g. "npbs" or "pbs"
    bf = bs_model_add_cmd[/--best_fit\s+([yn])/i, 1]  # "y" or "n"
    tag = [(bf&.downcase == 'y' ? 'bf' : nil), bs].compact.join('.')
    outdir = File.join(outdir0, bs_model+'+bs_inBV'+'.'+tag)

    sub_cpu = (cpu.to_f/bs_models.size).to_i
    cmd = "#{RUBY} #{CREATE_HESSIAN_BS} --ali #{phylip} --calibrated_tree #{species_tree} --outdir #{outdir} --force --ref #{ref_tre} -b #{bootstrap} --cpu #{sub_cpu} -m #{bs_model} --mcmctree_ctl #{mcmctree_ctl} --run_mcmctree #{cmd_add} #{bs_model_add_cmd}"
    ` #{cmd} >/dev/null `
    ` mv #{outdir}/mcmctree #{outdir}/combined `
    extract_hessian("#{outdir}/combined/in.BV", "#{outdir}/combined/hessian")
  end
end


def extract_hessian(src, dst)
  return unless File.exist?(src)
  lines = File.readlines(src)
  i = lines.index { |l| l =~ /^Hessian/ }
  File.write(dst, i ? lines[(i + 2)..].to_a.join : "")
end
...
opts = GetoptLong.new(
  ...
  ['--drift', GetoptLong::REQUIRED_ARGUMENT],
...
s2_arg = s2 == '' ? '' : s2
cmd = " Rscript #{SIM_TREE} -n #{ntips} -m #{mu} --sd #{sd} --s2 #{s2_arg} --drift #{drift} --rgeneInf #{rgeneInf} -b #{birth} -d #{death} --age #{age} --rho #{rho} -o #{sim_tree_outdir} --clock #{sim_clock} #{sim_tree_arg_add} "
p cmd
...
models.uniq!
# get bs_models
models, bs_models, bs_model_add_cmds = get_bs_models(models)
...
Parallel.map(models, in_processes: cpu) do |model|
  ...
  if model == 'ori'
    ...
    do_create_hessian(bs_models, bs_model_add_cmds, bootstrap, phylip, species_tree, mcmctree_ctl, ref_tre,
                      (cpu / 2).floor, outdirs[:dating], is_mwopt)
  else
    ph_sub_outdir = File.join(sub_outdir, 'ph')
    if model =~ /\+fd/
      hessian_type = 'fd'
      model = model.split('+').reject{|i|i == 'fd'}.join('+')
    end
    run_phyloHessian(ori_fasta, ori_unrooted_tree, ph_sub_outdir, ref_tre, real_cpu, model, bd, bsn, clock, sim_ali_outdir, is_mwopt, sub_cmd_out_fh, hessian_type)
    `rm -rf #{sub_outdir}/combined*; mv #{ph_sub_outdir}/date/* #{sub_outdir}`
  end

  # adjust source filename if needed
  src = File.join(sub_outdir, 'combined', 'in.BV')
  dst = File.join(sub_outdir, 'combined', 'hessian')
  extract_hessian(src, dst)
