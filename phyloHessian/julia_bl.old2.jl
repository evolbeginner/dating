...
function do_phylo_log_lk(
    ...
    # invariable-site handling
    rs2 = rs
    props2 = props_norm
    is_inv_site = false
    if is_do_inv
        @views tip_liks = liks_ori[:, 1:nb.tip]
        gap_cols = all(tip_liks .== 1.0, dims=2)
        @views res = tip_liks[.!gap_cols[:], :]
    ...
    @inbounds for r_i in eachindex(rs2), q_i in eachindex(q_pis)
        # LG4M/LG4X coupled rate-class behavior
        if is_lg4 && r_i != q_i
            continue
        end
        counter += 1
        ...
        j = 0
        for anc in (nb.tip + nb.node):-1:(1 + nb.tip)
            ...
            for c in children
                ...
                # If your decomposition can be complex in some model, keep fallback:
    ...
    if is_do_inv
        if is_inv_site
			#println(logsumexp(log.(weights) .+ lks))
            return logsumexp(log.(weights) .+ lks)
        else
            lks[isnan.(lks)] .= 0.0
            inv_weights = weights .* (1 - inv_prop)
			#println(logsumexp(log.(inv_weights) .+ lks))
            return logsumexp(log.(inv_weights) .+ lks)
        end
    else
        return logsumexp(log.(weights) .+ lks)
    end
