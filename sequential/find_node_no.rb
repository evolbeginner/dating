...
opts = GetoptLong.new(
  ...
  ['--output_numbered_tree', '--ont', GetoptLong::NO_ARGUMENT],
)
...
tree.internal_nodes.each do |node|
  case type
    when 'gamma'
      node.bootstrap_string = "\"G(" + node_name_2_paras[node.bootstrap.to_s].join(',') + ")\""
    when 'st'
      node.bootstrap_string = "\"ST(" + node_name_2_paras[node.bootstrap.to_s].join(',') + ")\""
    when 'sn'
      node.bootstrap_string = "\"SN(" + node_name_2_paras[node.bootstrap.to_s].join(',') + ")\""
  end
end
