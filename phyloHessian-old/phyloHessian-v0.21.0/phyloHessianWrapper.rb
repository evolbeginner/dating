...
MFATOPHY = File.expand_path( File.join(ADD_SCRIPTS_DIR, 'MFAtoPHY.pl') )
...
def run_iqtree(treefile, model, pmsf, seqfile, outdir, blmin, iqtree_add_arg0, iqtree_add_arg, cpu)
  if ! pmsf.nil?
    `#{IQTREE} -te #{treefile} -m #{model} -s #{seqfile} -pre #{outdir}/iqtree -redo -wsl -quiet #{iqtree_add_arg0} -T #{cpu}`
  end
  `#{IQTREE} -blfix #{treefile} -m POISSON -s #{seqfile} -pre #{outdir}/iqtree -redo -wsl -quiet #{iqtree_add_arg0} -T #{cpu}`
  `#{IQTREE} -te #{outdir}/iqtree.treefile -m #{model} -s #{seqfile} -pre #{outdir}/iqtree -redo -wsl -quiet #{iqtree_add_arg} -blmin #{blmin} -T #{cpu}`
end
...
def run_phyml(treefile, model, pmsf, seqfile, outdir, blmin, iqtree_add_arg0, iqtree_add_arg, cpu)
  ...
  FileUtils.cp_r(create_phylip(seqfile), './'+getCorename(seqfile)+'.phy')
  phylip = [getCorename(seqfile), 'phy'].join('.')

  out_fh = File.open("phyml.cmd", 'w')
  ...
  if ` #{cmd} 2>/dev/null`
    ...
    out_fh.puts cmd2
    ` #{cmd2} ` || raise("Both PHYML commands failed!")
  end
  out_fh.close
end
...
def iqtree_to_phyml(model)
  ...
  parts << "-m #{sm}" if sm
  order.each { |blk|
    blk == :I ? parts << "-v #{iv || 'e'}" : parts.concat(["-c #{gk || 4}", "-a #{ga || 'e'}"])
  }
  parts.join(' ')
end
